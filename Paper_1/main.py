...
# Coulomb regularisation
epsilon = 1e-15  # Regularisation to stop potential energy singularity
...
# # Create relative momementum operator
# p_rel=ql.create_relative_momentum_matrix(N, delta_X)

# expectation_value = np.dot(np.conj(eigenvector), np.dot(p_rel.toarray(), eigenvector))
...
# Calculate leakage
# leakage = ql.calculate_leakage(psi_2D_t[1], N, N_ext)
leakage = qf.calculate_leakage(X, Y, q_1, q_2, d, L, psi_2D_t, N, N_ext)


# # Create extended relative momementum time vector
# p_rel_ext=ql.create_relative_momentum_matrix(N_ext, delta_X_ext)
# relative_expected_momentum_t = ql.solve_future_expected_relative_momentum(num_time_steps+1, psi_1D_t, p_rel_ext)

# New line by VA for squared raltive momentum
# relative_expected_momentum_squared_t = ql.solve_future_expected_relative_momentum_squared(num_time_steps + 1, psi_1D_t, p_rel_ext)
